...
```{r loadprocess, echo=FALSE, message=FALSE, warning=FALSE}
library(lubridate)
library(httr)
library(dplyr)
library(ggplot2)
...
```{r fuel usage, fig.width=12, echo=TRUE}
...
```{r fuelcorrelation, echo=TRUE, dpi=300, fig.width=6, fig.height=5}
fuels = names(d)[-8]
...
```{r fuelcorrelationhighres, echo=FALSE, dpi=300, fig.width=6, fig.height=5, }
fuels = names(d)[-8]
cor_mat = matrix(NA, nrow = length(fuels), ncol = length(fuels))
...
```{r loadprocesscode, echo=TRUE, eval=FALSE}
library(lubridate)
library(httr)
library(dplyr)
